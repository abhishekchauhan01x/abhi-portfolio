...
const Projects = () => {
  ...
  return (
    <div className="py-2">
      <h2 className="text-4xl font-bold text-center mb-12 bg-[#F0EBD8] bg-clip-text text-transparent font-merriweather">
        Projects
      </h2>
      ...
      {/* <div className="grid grid-cols-1 md:grid-cols-2 gap-6"> */}
        ...
        {projects.map((project) => (
          <div
            key={project.id}
            className="group relative overflow-hidden rounded-xl bg-slate-900/60 backdrop-blur-sm border border-slate-700/50 hover:border-slate-600/50 transition-all duration-300 hover:shadow-lg hover:shadow-slate-900/50 p-6"
          >
            {/* Title */}
            <h3 className="text-xl font-bold text-white mb-3 font-merriweather group-hover:text-cyan-400 transition-colors duration-300">
              {project.title}
            </h3>
            
            {/* Description */}
            <p className="text-gray-300 text-sm leading-relaxed mb-4 font-roboto">
              {project.description}
            </p>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {project.technologies.map((tech, index) => (
                <span
                  key={index}
                  className="px-3 py-1.5 text-xs font-medium bg-slate-700/70 text-slate-300 rounded-md border border-slate-600/30 hover:bg-slate-600/70 hover:text-cyan-300 transition-colors duration-200 font-roboto"
                >
                  {tech}
                </span>
              ))}
            ...
            <div className="flex gap-6">
              <button
                ...
                className="flex items-center gap-2 text-slate-300 hover:text-cyan-400 text-sm font-medium transition-colors duration-200 font-roboto group/btn"
              >
